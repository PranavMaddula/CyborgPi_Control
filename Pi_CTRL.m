%{
TODO
1. Store and pass rpi/cam object - done
2. set image in start button - ?
3. auto update image from pi - done
4. Acquire and plot data from upd stream
...
function varargout = Pi_CTRL(varargin)
% PI_CTRL - Execute graphic updates at regular intervals
%   MATLAB code for Pi_CTRL.fig
%      PI_CTRL, by itself, creates a new PI_CTRL 
%      or raises the existing singleton*.
%
%      H = PI_CTRL returns the handle to a new PI_CTRL
%      or the handle to the existing singleton*.
%
%      PI_CTRL('CALLBACK',hObject,eventData,handles,...) calls
%      the local function named CALLBACK in PI_CTRL.M with 
%      the given input arguments.
%
%      PI_CTRL('Property','Value',...) creates a new 
%      PI_CTRL or raises the existing singleton*.
%      Starting from the left, property value pairs are applied to the 
%      GUI before Pi_CTRL_OpeningFcn gets called.
%      An unrecognized property name or invalid value makes property
%      application stop.  All inputs are passed to 
%      Pi_CTRL_OpeningFcn via varargin.
%
...
% Last Modified by GUIDE v2.5 08-Aug-2019 16:20:16

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Pi_CTRL_OpeningFcn, ...
                   'gui_OutputFcn',  @Pi_CTRL_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Pi_CTRL is made visible.
function Pi_CTRL_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Pi_CTRL (see VARARGIN)

% Choose default command line output for Pi_CTRL
...
% % handles.surf = surf(handles.axes1,peaks);

% handles.rpi = 'This is a new field in the handles structure.';
setappdata(handles.edit1,'RasPi',0); 

 handles.timer1 = timer('Name','PiCamera', ...
       'Period',.05,                     ... 
       'StartDelay',0,                   ... 
       'TasksToExecute',inf,             ... 
 ...
 handles.timer2 = timer('Name','Intan', ...
    'Period',1,                     ... 
    'StartDelay',1,                   ... 
    'TasksToExecute',inf,             ... 
    'ExecutionMode','fixedSpacing',   ...
    'TimerFcn',{@update_intan,hObject}); 
...
function varargout = Pi_CTRL_OutputFcn(hObject, eventdata, handles) 
...
ip = get(handles.edit1,'String');
 ...
 if ~contains(ip,'.')
   fprintf('Error: Please Enter a Valid IP\n');
 else
     usr = 'pi'
     pass = 'ramanlab'
     ...
     setappdata(handles.edit1,'RasPi',rpi); 
     setappdata(handles.edit1,'PiCam',cam);   
     
%      u = udp('192.168.1.93', 4407, 'LocalPort',4407);
%      u.InputBufferSize = 4096
%      u.ReadAsyncMode = 'continuous';
%      fopen(u);
%      
%      setappdata(handles.edit1,'UDP',u); 
      
     
     if strcmp(get(handles.timer1, 'Running'), 'off')
     ...
     [s, status] = urlread('http://192.168.1.93', 'post', {'__SL_P_UDI', 'S0'});
...
rpi = getappdata(handles.edit1,'RasPi');
system(rpi, 'echo "1" > CyborgPi/command.txt')

if strcmp(get(handles.timer1, 'Running'), 'on')
    stop(handles.timer1);
end
...
[s, status] = urlread('http://192.168.1.93', 'post', {'__SL_P_UDI', 'C0'});
...
% A = imread('MSL10 SNPs Updated.PNG');
% imshow(A, 'Parent', handles.axes2); % axes1 in axes2, NOT axes1.

cam = getappdata(handles.edit1,'PiCam');
img = snapshot(cam);
imshow(img, 'Parent', handles.axes2); % Display in axes2, NOT axes1.
drawnow;


% START USER CODE
function update_intan(hObject,eventdata,hfigure)
% Timer timer2 callback, called each time timer iterates.
...
read_Intan_RHD2000_file(path,handles)

t_amplifier = getappdata(handles.edit2,'t_amplifier');
amplifier_data = getappdata(handles.edit2,'amplifier_data');

plot(handles.axes1,t_amplifier, amplifier_data(1,:))
[s, status] = urlread('http://192.168.1.93', 'post', {'__SL_P_UDI', 'S0'});


% plot(axes1,data);

% data = getappdata(handles.edit1,'IntanData');
% u = getappdata(handles.edit1,'UDP');
% 
...
% setappdata(handles.edit1,'IntanData',data);
...
setappdata(handles.edit1,'RasPi',0); 
setappdata(handles.edit1,'PiCam',0); 
% END USER CODE
...
function read_Intan_RHD2000_file(path,handles,varargin)
...
%path = 'C:\Users\Pranav Maddula\Documents\RamanLab\intan\';
% d = dir([path '*.rhd']);
% file = d(end).name;

tic;

file = getlatestfile(path)
filename = [path,file];
...
move_to_base_workspace(notes,handles);
move_to_base_workspace(frequency_parameters,handles);
if (data_file_main_version_number > 1)
    move_to_base_workspace(reference_channel,handles);
end

if (num_amplifier_channels > 0)
    move_to_base_workspace(amplifier_channels,handles);
    if (data_present)
        move_to_base_workspace(amplifier_data,handles);
        move_to_base_workspace(t_amplifier,handles);
    end
    move_to_base_workspace(spike_triggers,handles);
end
if (num_aux_input_channels > 0)
    move_to_base_workspace(aux_input_channels,handles);
    if (data_present)
        move_to_base_workspace(aux_input_data,handles);
        move_to_base_workspace(t_aux_input,handles);
    end
end
if (num_supply_voltage_channels > 0)
    move_to_base_workspace(supply_voltage_channels,handles);
    if (data_present)
        move_to_base_workspace(supply_voltage_data,handles);
        move_to_base_workspace(t_supply_voltage,handles);
    end
end
if (num_board_adc_channels > 0)
    move_to_base_workspace(board_adc_channels,handles);
    if (data_present)
        move_to_base_workspace(board_adc_data,handles);
        move_to_base_workspace(t_board_adc,handles);
    end
end
if (num_board_dig_in_channels > 0)
    move_to_base_workspace(board_dig_in_channels,handles);
    if (data_present)
        move_to_base_workspace(board_dig_in_data,handles);
        move_to_base_workspace(t_dig,handles);
    end
end
if (num_board_dig_out_channels > 0)
    move_to_base_workspace(board_dig_out_channels,handles);
    if (data_present)
        move_to_base_workspace(board_dig_out_data,handles);
        move_to_base_workspace(t_dig,handles);
    end
end
if (num_temp_sensor_channels > 0)
    if (data_present)
        move_to_base_workspace(temp_sensor_data,handles);
        move_to_base_workspace(t_temp_sensor,handles);
    end
end
...
function move_to_base_workspace(variable,handles)
...
setappdata(handles.edit2,variable_name,variable); 
