...
def run(leftspeed, rightspeed):
    ...
    pwm_ENA.ChangeDutyCycle((leftspeed+1))
...
def ramp(speed, rate):
    ...
    GPIO.output(IN3, GPIO.HIGH)
    GPIO.output(IN4, GPIO.LOW)
    pwm_ENA.ChangeDutyCycle((speed+2)/2)
    pwm_ENB.ChangeDutyCycle((speed-3)/2)
    time.sleep(rate)
    pwm_ENA.ChangeDutyCycle(speed+2)
    pwm_ENB.ChangeDutyCycle((speed-3))
...
def pauserev(leftspeed, rightspeed, wait):
    ...
    GPIO.output(IN3, GPIO.LOW)
    GPIO.output(IN4, GPIO.HIGH)
    pwm_ENA.ChangeDutyCycle((leftspeed+2))
    pwm_ENB.ChangeDutyCycle((rightspeed-3))
    time.sleep(.02)
